...
class Prism_Clarrise_Integration(object):
	def __init__(self, core, plugin):
		self.core = core
		self.plugin = plugin

		if platform.system() == "Windows":
			self.examplePath = os.environ["appdata"] + "\\Isotropix\\Clarisse\\4.0"
		...
		#TODO::
			...
			self.examplePath = os.path.join("/home", userName, "Clarrise", "2019")
		elif platform.system() == "Darwin":
			userName = os.environ['SUDO_USER'] if 'SUDO_USER' in os.environ else os.environ['USER']
			self.examplePath = "/Users/%s/Library/Preferences/Autodesk/Clarrise/2019" % userName
	...
	def getExecutable(self):
		execPath = ""
		if platform.system() == "Windows":
			defaultpath = os.path.join(self.getClarrisePath(), "clarisse.exe")
	...
	@err_decorator
	def getClarrisePath(self):
		
		#TODO:: implement maya's ways of getting all avaiable version from the registry

		installDir = "C:\\Program Files\\Isotropix\\Clarisse iFX 4.0 SP2\\Clarisse"
	...
	def integrationAdd(self, origin):
		path = QFileDialog.getExistingDirectory(self.core.messageParent, "Select Clarisse folder", os.path.dirname(self.examplePath))

		if path == "":
			return False

		result = self.writeClarriseFiles(path)
	...
	def writeClarriseFiles(self, pluginPath):
		try:
			...
			msgStr = "Errors occurred during the installation of the Clarrise integration.\nThe installation is possibly incomplete.\n\n%s\n%s\n%s" % (str(e), exc_type, exc_tb.tb_lineno)
	...
	def removeIntegration(self, installPath):
		try:
			...
			msgStr = "Errors occurred during the removal of the Clarrise integration.\n\n%s\n%s\n%s" % (str(e), exc_type, exc_tb.tb_lineno)
			msgStr += "\n\nRunning this application as administrator could solve this problem eventually."
	...
	# locfile is installLocations.init which store the paths to all DCCs avaiables
	# result = {} empty dic to be filled from here
	# 
	def installerExecute(self, pluginItem, result, locFile):
		try:
			...
			for i in pluginPaths:
				result["Clarrise integration"] = self.writeClarriseFiles(i)
				if result["Clarrise integration"]:
